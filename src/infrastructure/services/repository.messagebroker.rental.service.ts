...
export class RepositoryAndMessageBrokerRentalService implements IRentalService {
  ...
  public async request(body: any): Promise<Request> {
    ...
    const requestSended = await requestRepository.sendRequest(request);

    // Also publish it as an message
    const messagePublisher = await this.getMessagePublisher();
    await messagePublisher.publishMessage(
      MessageType.RentalRequested,
      requestSended
    );
  ...
  public async accept(id: string, body: any): Promise<boolean> {
    ...
    const acceptedRentalRequest = await rentalRepository.acceptRequest(
      id,
      request
    );
    ...
    const messagePublisher = await this.getMessagePublisher();
    await messagePublisher.publishMessage(
      MessageType.RentalAccepted,
      acceptedRentalRequest
    );
  ...
  public async decline(id: string, body: any): Promise<boolean> {
    ...
    const DeclinedRequest = await rentalRepository.declineRequest(id, request);

    // Also publish it as an message
    const messagePublisher = await this.getMessagePublisher();
    await messagePublisher.publishMessage(
      MessageType.RentalDeclined,
      DeclinedRequest
    );
